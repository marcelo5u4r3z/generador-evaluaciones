...
function validateChatResponse(result) {
  ...
  if (result.artifact) {
    if (!ARTIFACT_TYPES.has(result.artifact.type)) {
      throw new Error('Provider returned an invalid artifact type.');
    }
    if (!result.artifact.title || !result.artifact.content) {
      throw new Error('Provider returned an incomplete artifact.');
    }
    if (!['markdown-latex', 'plain-text'].includes(result.artifact.format)) {
      throw new Error('Provider returned an invalid artifact format.');
    }
  }
...
function createChatHandler(provider, { logger = console, model = 'unknown' } = {}) {
  if (!provider?.generate) {
    throw new Error('An AI provider is required.');
  }
