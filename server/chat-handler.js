...
function safeProviderError(error) {
  ...
  return {
    ...
    providerCode: error?.code || error?.type || null,
...
function createChatHandler(provider, { logger = console, model = 'unknown' } = {}) {
  ...
  return async function chatHandler(body) {
    ...
    try {
      ...
      logger.error(JSON.stringify({ timestamp: new Date().toISOString(), operation: 'chat', model, success: false, latencyMs: Date.now() - startedAt, ...safeProviderError(error) }));
      return { status: 503, body: { error: 'provider_unavailable', message: 'No pude preparar esto ahora. Probá nuevamente.' } };
    }
  };
}

module.exports = { createChatHandler, validateChatRequest, validateChatResponse, safeProviderError };
