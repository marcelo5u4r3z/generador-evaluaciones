...
function renderAgent(course) {
  ...
  const modeStatus = NERIO_CONFIG.mode === 'mock' ? '<span class="demo-status">Demostración</span>' : '';
  const modeNote = NERIO_CONFIG.mode === 'mock' ? '<p class="composer-note">Las respuestas están preparadas para esta demostración.</p>' : '<p class="composer-note">Nerio utiliza el contexto académico de este curso.</p>';
  app.innerHTML = pageShell(`<section class="agent-layout ${state.draft ? 'has-document' : ''}"><section class="chat-panel"><div class="agent-context"><span class="nerio-presence">${icon('nerio')}</span><div><p class="eyebrow">NERIO · Curso activo</p><h1>${escapeHtml(course.subject)}</h1><p>${escapeHtml(course.level)} · ${escapeHtml(course.group)}</p></div>${modeStatus}</div><div class="conversation-intro"><h2>¿Qué preparamos?</h2><p>Podés escribirlo como se lo dirías a alguien que ya conoce tu curso.</p></div><div class="quick-actions">${actions}</div><div class="messages" id="messages">${messages}${activity}</div><form id="chat-form" class="composer"><textarea id="chat-input" rows="1" placeholder="Por ejemplo: Preparame la clase del viernes…" aria-label="Escribile a Nerio" ${state.requestStatus === 'sending' ? 'disabled' : ''}></textarea><button aria-label="Pedirle a Nerio" ${state.requestStatus === 'sending' ? 'disabled' : ''}>${icon('send')}</button></form>${modeNote}</section>${state.draft ? documentPanel(state.draft) : `<aside class="document-placeholder"><span class="document-watermark">N</span><p class="eyebrow">Mesa de trabajo</p><h2>Lo que prepares con Nerio va a aparecer acá</h2><p>Podrás revisarlo, editarlo y llevarlo a clase.</p></aside>`}</section>`, { course, active: 'agent', className: 'agent-page' });
